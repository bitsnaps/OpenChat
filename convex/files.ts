...
import { UPLOAD_ALLOWED_MIME, UPLOAD_MAX_BYTES } from "../src/lib/config/upload";
import { ERROR_CODES } from "../src/lib/error-codes";
import { sanitizeAndValidateFileName } from "../src/lib/filename";
...
export const saveFileAttachment = action({
  ...
  handler: async (ctx, args): Promise<SavedAttachment> => {
    ...
    while (
      ...
      (!meta || meta.size === null || meta.size === undefined || !meta.contentType)
    ) {
    ...
    if (!meta || meta.size === null || meta.size === undefined || !meta.contentType) {
...
export const saveGeneratedImage = action({
  ...
  handler: async (ctx, args): Promise<SavedAttachment> => {
    ...
    const attachmentId = await ctx.runMutation(internal.files.internalSaveGenerated, {
      chatId: args.chatId,
      key: args.key,
      fileName,
      fileType: mime,
