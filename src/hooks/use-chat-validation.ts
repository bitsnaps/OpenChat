...
import { getModelProvider, isModelPremium, requiresUserApiKey } from "@/lib/model-utils";
...
export function useChatValidation() {
  ...
  const validateSearchQuery = useCallback((query: string) => validateQueryParam(query), []);
