...
export const CONNECTOR_CONFIGS: Record<ConnectorType, ConnectorConfig> = {
  gmail: {
    ...
    authConfigId: process.env.VITE_GMAIL_AUTH_CONFIG_ID ?? "gmail_oauth",
  },
  googlecalendar: {
    type: "googlecalendar",
    ...
    authConfigId: process.env.VITE_CALENDAR_AUTH_CONFIG_ID ?? "googlecalendar_oauth",
  },
  googledrive: {
    type: "googledrive",
    ...
    authConfigId: process.env.VITE_GOOGLE_DRIVE_AUTH_CONFIG_ID ?? "googledrive_oauth",
  },
  notion: {
    type: "notion",
    displayName: "Notion",
    ...
    authConfigId: process.env.VITE_NOTION_AUTH_CONFIG_ID ?? "notion_oauth",
  },
  googledocs: {
    type: "googledocs",
    ...
    authConfigId: process.env.VITE_GOOGLE_DOCS_AUTH_CONFIG_ID ?? "googledocs_oauth",
  },
  googlesheets: {
    type: "googlesheets",
    ...
    authConfigId: process.env.VITE_GOOGLE_SHEETS_AUTH_CONFIG_ID ?? "googlesheets_oauth",
  },
  slack: {
    type: "slack",
    displayName: "Slack",
    ...
    authConfigId: process.env.VITE_SLACK_AUTH_CONFIG_ID ?? "slack_oauth",
  },
  linear: {
    type: "linear",
    displayName: "Linear",
    ...
    authConfigId: process.env.VITE_LINEAR_AUTH_CONFIG_ID ?? "linear_oauth",
  },
  github: {
    type: "github",
    displayName: "GitHub",
    ...
    authConfigId: process.env.VITE_GITHUB_AUTH_CONFIG_ID ?? "github_oauth",
  },
  twitter: {
    type: "twitter",
    ...
    authConfigId: process.env.VITE_TWITTER_AUTH_CONFIG_ID ?? "twitter_oauth",
  },
};
...
const getConnectorPatterns = (): Record<ConnectorType, string[]> => {
  const patterns: Record<ConnectorType, string[]> = {} as Record<ConnectorType, string[]>;
...
export const isConnectorTool = (toolName: string): boolean => {
  ...
  if (SUPPORTED_CONNECTORS.some((connectorType) => lowerToolName.includes(connectorType))) {
    return true;
  }
 ...
 */
export const getConnectorTypeFromToolName = (toolName: string): ConnectorType => {
  const lowerToolName = toolName.toLowerCase();
